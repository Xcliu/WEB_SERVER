...
class func_with_time_out{
    ...
    public:
        ...
        std::pair<func_status,return_data> run_func(int64_t tiemout_ms, const std::function<R(Args...)> f, Args&&... args){
            auto task_future = std::async(std::launch::async,f,std::forward<Args...>(args...));
