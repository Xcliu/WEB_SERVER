...
class Timer{
    public:
        using time_point_t = typename std::chrono::time_point<std::chrono::high_resolution_clock>;
        explicit Timer() = default;
        virtual ~Timer() = default;
        void save_time_point() {
            time_point_array_[latest_update_index_%2] = std::chrono::high_resolution_clock::now();
...
class Scope_timer: public Timer{
    public:
        ...
        virtual ~Scope_timer(){
            save_time_point();
            output_obj_ << "cost: " << cal_time_interval_ms().count() << " ms." << std::endl;
        }
    public:
