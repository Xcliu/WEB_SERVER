...
class Write_read_lock {
 ...
 public:
  explicit Write_read_lock(bool _priority = READER_PREFER)
...
class Writer_lock_guard {
 ...
 public:
  ...
  virtual ~Writer_lock_guard() { wdl.writer_unlock(); }
};
...
class Reader_lock_guard {
 ...
 public:
  Reader_lock_guard(Write_read_lock &_wdl) : wdl(_wdl) { wdl.reader_lock(); }
  virtual ~Reader_lock_guard() { wdl.reader_unlock(); }
};
