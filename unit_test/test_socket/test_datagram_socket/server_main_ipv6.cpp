...
int main(){
    ...
    try{
        std::shared_ptr<server_socket_datagram> server_socket_obj{new server_socket_datagram(AF_INET6)};
        server_socket_obj->init_socket();
        // in6_addr temp=in6addr_any;
        // char dst[sizeof(temp)];
        // inet_ntop(AF_INET6,&(temp),dst,sizeof(temp));
        server_socket_obj->bind_socket_to_ipv6_port("fec0:da24:bdff:fe76:cea2",8888);
        while(true){
