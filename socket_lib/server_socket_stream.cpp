...
void server_socket_stream::accept_client_request(){
    ...
    if(ip_version_ == AF_INET){
        ...
        server_connected_socket_fd_=accept(server_socket_fd_,(struct sockaddr*)temp.get(),&client_addrLength);
...
void server_socket_stream::send_short_mesg(const std::string &str){
    ...
    int s = send(server_connected_socket_fd_,str.c_str(),str.size(),0);//发送响应
    if ( s<0 ){
        systemcall_error_info();
