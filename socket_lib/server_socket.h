...
class server_socket {
 ...
 public:
  server_socket(decltype(AF_INET) ip_version = AF_INET,
                decltype(SOCK_STREAM) socket_type = SOCK_STREAM)
  ...
  void bind_socket_to_ipv6_port(const char* ipv6_address = "",
                                int server_port = 0);
  virtual const std::string& receive_data_from_client(
      bool save_client_addr) = 0;
