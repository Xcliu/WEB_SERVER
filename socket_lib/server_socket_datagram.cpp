...
const std::string &server_socket_datagram::receive_data_from_client(bool save_client_addr){
    ...
    if(save_client_addr){       
        sockaddr *src_ptr = ip_version_==AF_INET
                            ?(sockaddr *)(client_socket_info_ipv4_->get_sockaddr().get())
                            :(sockaddr *)(client_socket_info_ipv6_->get_sockaddr().get());
    }
    ssize_t mesg_len=recvfrom(server_socket_fd_,temp,max_buffer_size_,0,src_ptr,&src_len);
...
void server_socket_datagram::send_short_mesg(const std::string &str){
    ...
    sockaddr *dest_ptr= ip_version_==AF_INET
                        ?(sockaddr *)(client_socket_info_ipv4_->get_sockaddr().get())
                        :(sockaddr *)(client_socket_info_ipv6_->get_sockaddr().get());



    ssize_t mesg_len = sendto(server_socket_fd_,str.c_str(),str.size(),0,dest_ptr,dest_len);//发送响应
    if(mesg_len==max_buffer_size_){
