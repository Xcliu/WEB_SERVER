...
//     if(tag<0){
//         pro.set_exception(
//             std::make_exception_ptr(std::logic_error("Error: input for sqrt
//             should be positive."))
//         );
//     }else{
...
//         return in*in;
//     });
//     auto fut=pac.get_future();//由于pac 传递给thread_producers是被销毁，因此
//     fut 必须在生成pac 以后马上获得
//     //而不能放在子线程中获取。
...
//         }
//     );
//     std::thread thread_producers(std::move(pac),-3.14);// convert to right
//     value is necessary. thread_consumer.join(); thread_producers.join();
//     return 0;
// }
...
int main(int argc, char *argv[]) {
  ...
  auto fut = std::async(
      std::launch::async,
      [](double in) -> double {
        std::cout << "task thread ID: " << std::this_thread::get_id()
                  << std::endl;
        sleep(3);
        return in * in;
      },
  ...
  std::thread thread_c1([&fut_c1, &mtx]() {
    std::unique_lock<std::mutex> ul(
        mtx);  // the mutex used to protect outstream(a kind of shared
               // resources)
    std::cout << "thread ID: " << std::this_thread::get_id() << " ; "
              << fut_c1.get() << std::endl;
  });

  // auto fut_c2=fut.share(); //error in auto fut_c1=fut.share(); the
  // std::move() is default, so the fut instance is destoryed after auto
  // fut_c1=fut.share();
  auto fut_c2 = fut_c1;  //
